...
const OPENING_REFLECTION = `A veces pienso que nos está quedando una mierda de mundo.

No porque hayamos dejado de saber lo que está bien. Eso sería demasiado fácil.

Lo sabemos. Sabemos cuándo alguien tiene hambre. Sabemos cuándo una persona está sola. Sabemos cuándo una guerra está destruyendo una casa, cuándo alguien necesita ayuda, cuándo estamos siendo crueles.

Y aun así seguimos.

Seguimos deslizando el dedo. Seguimos comprando. Seguimos corriendo. Seguimos diciendo que no tenemos tiempo.

Quizá lo terrible de nuestra época no sea la ignorancia. Quizá sea habernos acostumbrado.

A ver demasiado.

A sentir demasiado poco.

Pero de vez en cuando ocurre algo extraño. Alguien escribe una frase. Otra persona la encuentra. Y durante unos segundos… vuelve a recordar que el otro existe.

Tal vez para eso todavía sirva la literatura.

No para salvar el mundo.

Pero quizá sí para impedir que terminemos de acostumbrarnos a perderlo.

Ahora sí. ¿De qué quieres hablar?`;
...
export default function GranAvatarExperience() {
  ...
  >([
    makeMessage(
      "assistant",
      "Hay algo que estaba pensando antes de que llegaras. Escúchame y después hablamos.",
    ),
  ]);
  const [messageDraft, setMessageDraft] = useState("");
  const [notice, setNotice] = useState(
    "Antes de hablar de literatura, escucha el pensamiento inicial del avatar.",
  );
  ...
  const playVoice = useCallback(
    async (text: string, mode: VoiceMode) => {
      ...
      setNotice(
        mode === "reading"
          ? "La voz y el rostro se preparan para habitar el poema."
          : "La presencia alinea su voz con el rostro.",
      );
        ...
        <section id="avatar" className={styles.avatarStage}>
          ...
          <div className={styles.readingControls}>
            <button
              type="button"
              onClick={() => {
                void playVoice(OPENING_REFLECTION, "speaking");
              }}
            >
              Pensamiento inicial
            </button>
