...
export default function OrganismoV47() {
    ...
    try {
      ...
      if (storedNursery) {
        ...
        if (Array.isArray(parsed)) {
          setSavedImpulses(parsed.slice(-24) as SymbolicImpulse[]);
        }
      }
    } catch {
  ...
  const symbolicImpulses = useMemo(
    () => buildSymbolicImpulses(symbolicNucleus, feedingText),
    [symbolicNucleus, feedingText],
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setActiveImpulse(first);
      setSymbolicPulse((value) => value + 1);
      setLastWord(first.value);
      setFeedingHabitat(first.habitat as Habitat);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      setActiveImpulse(next);
      setSymbolicPulse((value) => value + 1);
      setLastWord(next.value);
      setFeedingHabitat(next.habitat as Habitat);
      setAbsorptionPulse((value) => value + 1);
      setImpulseAnatomyOpen(false);
  ...
  function recallImpulse(impulse: SymbolicImpulse) {
    setSymbolicNucleus(impulse.nucleus);
    revealImpulse(impulse);
  }
  ...
  return (
    <main className={styles.page}>
      ...
      >
        <summary>
          <span>VER DIRECCIÓN MUSICAL / PARTITURA GENÉTICA</span>
          <b>{engineOpen ? "—" : "+"}</b>
        </summary>
      ...
      <footer className={styles.footer}>
        <span>EMBRIÓN DIRECTOR 1.0 · CICLO POÉTICO RENOVABLE</span>
        <span>
