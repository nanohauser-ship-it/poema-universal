...
function currentDraftWord(text: string): string {
  return (
    text.match(/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:['’-][A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*$/)?.[0] ??
    ""
  );
}

function dominantHabitatFromText(text: string): Habitat {
  const words =
    text.match(/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:['’-][A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*/g) ??
    [];
  if (!words.length) return "memoria";

  const counts = Object.fromEntries(
    HABITATS.map((habitat) => [habitat, 0]),
  ) as Record<Habitat, number>;

  words.forEach((word) => {
    counts[semanticPower(word)] += 1;
  });

  return HABITATS.slice().sort((a, b) => counts[b] - counts[a])[0] ?? "memoria";
}

function digestionMessage(habitat: Habitat, word: string): string {
  const messages: Record<Habitat, string> = {
    deriva: "La membrana se volvió más líquida.",
    pulso: "El corazón sonoro aumentó su presión.",
    fractura: "Apareció una cicatriz nueva en la forma.",
    espectral: "La palabra dejó un halo alrededor del cuerpo.",
    maquina: "Una estructura rítmica comenzó a ensamblarse.",
    memoria: "El organismo guardó la palabra como reliquia.",
  };

  return word ? `${messages[habitat]} «${word}» está siendo digerida.` : messages[habitat];
}
...
export default function OrganismoV47() {
  ...
  const [erasePulse, setErasePulse] = useState(0);
  const [sentencePulse, setSentencePulse] = useState(0);
  const [keyCount, setKeyCount] = useState(0);
  const [lastIncomingWord, setLastIncomingWord] = useState("");
  const [digestion, setDigestion] = useState(
    "El teclado está conectado al cuerpo del organismo.",
  );
  ...
  const feedTextAreaRef = useRef<HTMLTextAreaElement | null>(null);
  ...
  const provisionalBiomass = useMemo(() => countWords(graft), [graft]);
  const draftWord = useMemo(() => currentDraftWord(graft), [graft]);
  const activeFeedingHabitat = useMemo(
    () => dominantHabitatFromText(graft || lastIncomingWord),
    [graft, lastIncomingWord],
  );
  const provisionalGrowth = Math.round(
    (1 - Math.exp(-provisionalBiomass / 18)) * 12,
  );
  const totalBiomass = (organism?.biomasaVerbal ?? 0) + provisionalBiomass;
  ...
  function spawnFeedingWord(word: string, habitat: Habitat) {
    const source = feedTextAreaRef.current?.getBoundingClientRect();
    const destination = chamberRef.current?.getBoundingClientRect();
    if (!source || !destination) return;

    const left = source.left + Math.min(source.width - 44, 30 + Math.random() * source.width * 0.55);
    const top = source.top + Math.min(source.height - 24, 20 + Math.random() * source.height * 0.5);
    const targetX = destination.left + destination.width * (0.48 + (Math.random() - 0.5) * 0.12);
    const targetY = destination.top + destination.height * (0.46 + (Math.random() - 0.5) * 0.16);
    const id = `${Date.now()}-${feedPulse}-${Math.random().toString(36).slice(2)}`;

    const particle: FlyingWord = {
      id,
      word,
      habitat,
      left,
    ...
    setFlyingWords((current) => [...current.slice(-10), particle]);
    const timer = window.setTimeout(() => {
      setFlyingWords((current) => current.filter((item) => item.id !== id));
    }, 1450);
    transitTimersRef.current.push(timer);
  }

  function receiveCompletedWord(word: string) {
    const clean = word.trim();
    if (!clean) return;
    const habitat = semanticPower(clean);

    setLastIncomingWord(clean);
    setDigestion(digestionMessage(habitat, clean));
    setFeedPulse((value) => value + 2);
    spawnFeedingWord(clean, habitat);

    const timer = window.setTimeout(() => {
      setLastIncomingWord((current) => (current === clean ? "" : current));
    }, 1700);
    transitTimersRef.current.push(timer);
  }

  function handleFeedingChange(nextRaw: string) {
    const next = nextRaw.slice(0, 1200);
    const previous = graft;

    if (next.length > previous.length) {
      const lastCharacter = next.at(-1) ?? "";
      if (/\s|[.,;:!?…]/.test(lastCharacter)) {
        const beforeDelimiter = next.slice(0, -1);
        const completed = beforeDelimiter.match(
          /[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:['’-][A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*$/,
        )?.[0];
        if (completed) receiveCompletedWord(completed);
      }
    }
  ...
  function handleFeedingKeyDown(
    ...
    if (event.key === "Backspace") {
      setErasePulse((value) => value + 1);
      setDigestion("La materia provisional se repliega antes de ser integrada.");
    }

    if (event.key === "Enter") {
      setSentencePulse((value) => value + 1);
      setDigestion("Una nueva respiración divide la materia en otra escena.");
    }
  }
  ...
  function feed() {
    ...
    const matter = graft.trim();
    const words = countWords(matter);
    const habitat = dominantHabitatFromText(matter);
    const next = injertar(organism, matter);

    persist(next);
    setGraft("");
    setLastIncomingWord("");
    setSentencePulse((value) => value + 2);
    setDigestion(
      `${words} palabras fueron integradas. ${digestionMessage(habitat, "")}`,
    );
    notify(
      `${words} palabras aumentaron la biomasa del organismo y activaron ${HABITAT_NAMES[habitat]}.`,
    );
  }
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.feedingTransitLayer} aria-hidden="true">
        {flyingWords.map((item) => (
          <span
            key={item.id}
            className={styles.flyingWord}
      ...
      <section className={styles.mainGrid}>
        <aside className={styles.readingPanel}>
          ...
          />
          <MetricBlock
            label="Biomasa verbal"
            value={String(totalBiomass)}
            caption={
              provisionalBiomass
                ? `+${provisionalBiomass} palabras sin integrar`
                : `${organism.alimentaciones} alimentaciones`
            }
          />
          ...
          <article className={styles.evolutionChamber}>
            ...
            <div
              ...
              className={[
                ...
                provisionalBiomass ? styles.chamberReceiving : "",
              ]
                .filter(Boolean)
                .join(" ")}
            >
              <EmbryoScene
                ...
                feedPulse={feedPulse}
                erasePulse={erasePulse}
                sentencePulse={sentencePulse}
                provisionalBiomass={provisionalBiomass}
                activeHabitat={activeFeedingHabitat}
                incomingWord={lastIncomingWord || draftWord}
              />

              <div
                className={styles.biomassGauge}
                data-active={provisionalBiomass ? "true" : "false"}
              >
                <small>BIOMASA EN FORMACIÓN</small>
                <strong>+{provisionalGrowth}%</strong>
                <i>
                  <b style={{ width: `${Math.min(100, provisionalBiomass * 4)}%` }} />
                </i>
              </div>
        ...
        <aside className={styles.rightColumn}>
          ...
          <section
            className={styles.interventionPanel}
            data-feeding={provisionalBiomass ? "true" : "false"}
          >
            <header className={styles.feedingPanelHeader}>
            ...
            <div className={styles.feedingMetrics}>
              <article>
                <small>BIOMASA TOTAL</small>
                <strong>{totalBiomass}</strong>
                <span>palabras</span>
              </article>
              <article>
                <small>MATERIA NUEVA</small>
                <strong>+{provisionalBiomass}</strong>
                <span>sin integrar</span>
              </article>
              <article>
            ...
            <div className={styles.feedingHabitat} data-habitat={activeFeedingHabitat}>
              <span>{HABITAT_GLYPHS[activeFeedingHabitat]}</span>
              <div>
                <small>ÓRGANO ACTIVADO</small>
                <strong>{HABITAT_NAMES[activeFeedingHabitat]}</strong>
              </div>
              <i />
            </div>

            <div className={styles.feedingTextArea}>
              <textarea
                ref={feedTextAreaRef}
                value={graft}
                onKeyDown={handleFeedingKeyDown}
                onChange={(event: ChangeEvent<HTMLTextAreaElement>) => handleFeedingChange(event.target.value)}
                placeholder="Escribe palabras para alimentar el organismo…"
                spellCheck="true"
              />
              <span className={styles.keyboardSignal}>
                {draftWord || lastIncomingWord || "ESCUCHANDO"}
              </span>
            </div>

            <div className={styles.digestionNarrative}>
              <small>RESPUESTA DEL CUERPO</small>
              <p>{digestion}</p>
            </div>

            <div className={styles.feedingLegend}>
              <span>LETRA · LATIDO</span>
              <span>ESPACIO · ABSORCIÓN</span>
              <span>ENTER · RESPIRACIÓN</span>
              <span>BORRAR · REPLIEGUE</span>
            </div>
      ...
      <details
        ...
        onToggle={(event: SyntheticEvent<HTMLDetailsElement>) =>
          setEngineOpen(event.currentTarget.open)
        }
      >
        <summary>
      ...
      <footer className={styles.footer}>
        <span>ORGANISMO V4.7 · ALIMENTACIÓN VERBAL</span>
        <span>
