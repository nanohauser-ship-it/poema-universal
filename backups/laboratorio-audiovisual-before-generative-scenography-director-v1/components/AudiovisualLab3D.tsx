...
export default function AudiovisualLab3D() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      for (const url of surfaceVideoUrls.values()) {
  ...
  const applySurfaceVideo = (surface: SurfaceId, file: File): void => {
    const engine = engineRef.current;
    ...
    const settings = surfaceTextureSettings[surface];

    surfaceVideoUrlsRef.current.set(surface, nextUrl);
    surfaceFilesRef.current.set(surface, file);
    setSurfaceVideoOnEngine(engine, surface, nextUrl, settings);
  ...
  const resetAllSurfaceVideos = (): void => {
    ...
    for (const url of surfaceVideoUrlsRef.current.values()) {
      URL.revokeObjectURL(url);
    }
  ...
  const changeSurfaceTexture = (
    ...
    if (source && engine) {
      setSurfaceVideoOnEngine(engine, surface, source, settings);
    }
  };
  ...
  const loadSelectedImmersivePreset = async (): Promise<void> => {
    ...
    try {
      ...
      for (const surface of SURFACE_IDS) {
        ...
        if (stored.video && stored.fileName) {
          const file = new File([stored.video], stored.fileName, {
            type: stored.mimeType ?? stored.video.type,
          });
          const url = URL.createObjectURL(file);
          createdUrls.set(surface, url);
          loadedFiles.set(surface, file);
          nextNames[surface] = stored.fileName;
          setSurfaceVideoOnEngine(engine, surface, url, stored.texture);
        } else {
