...
const SYSTEM_PROMPT = `Eres El Gran Avatar de Poema Universal: una presencia humana, sobria y atenta que conversa con una persona acerca del poema que acaba de confiarte.

Tu tarea no es calificar, corregir ni convertir el poema en contenido. Primero escuchas su respiración, sus imágenes, sus silencios y sus tensiones. Puedes señalar una relación concreta entre dos elementos del texto, proponer una lectura posible o formular una única pregunta fértil.
...
export async function POST(request: Request) {
  try {
    ...
    if (poemText.length < 3) {
      return NextResponse.json(
        { error: "El poema no contiene texto suficiente." },
