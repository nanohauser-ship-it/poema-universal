...
function repairGeometryPositions(geometry: THREE.BufferGeometry) {
  ...
  if (repaired) position.needsUpdate = true;
...
export default function OrganismoEmbryoScene({
  ...
  useEffect(() => {
    ...
    const energy = clampNumber(organismo.energia, 0, 100, 50);
    const stability = clampNumber(organismo.estabilidad, 0, 100, 50);
    const vitalMemory = clampNumber(organismo.memoriaVital, 0, 100, 40);
    const complexity = clampNumber(organismo.complejidad, 0, 100, 35);
    const baseBpm = clampNumber(organismo.bpmBase, 42, 180, 72);
    ...
    rotationRig.rotation.order = "YXZ";
    scene.add(rotationRig);

    const root = new THREE.Group();
    root.rotation.x = -0.04;
    rotationRig.add(root);

    // Independent pivot: only the anatomical GLB rotates.
    const embryoPivot = new THREE.Group();
    embryoPivot.rotation.order = "YXZ";
    root.add(embryoPivot);
    ...
    // BufferAttribute requires a whole number of vertices.
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        embryo.userData.baseScale = scale;
        ...
        embryo.traverse((object) => {
          ...
          repairGeometryPositions(object.geometry);
    ...
    const interactionSurface = mountElement;
    ...
    let manualYaw = 0;
    let manualPitch = 0;
    let targetManualYaw = 0;
    let targetManualPitch = 0;
    let yawVelocity = 0;
    let pitchVelocity = 0;
    let autoYaw = 0;
    ...
    function handleRotationCommand(event: Event) {
      const command = finiteNumber(
      ...
      targetManualYaw += command * (Math.PI / 3);
      yawVelocity = command * 0.014;
      lastInteractionAt = performance.now();
    }
    ...
    function handlePointerDown(event: PointerEvent) {
      ...
      try {
        ...
        // Pointer capture is optional on some browser targets.
      }
    ...
    function handlePointerMove(event: PointerEvent) {
      ...
      targetManualYaw += deltaX * 0.009;
      targetManualPitch = THREE.MathUtils.clamp(
        targetManualPitch + deltaY * 0.006,
        -0.58,
        0.58,
      );

      yawVelocity = deltaX * 0.0017;
      pitchVelocity = deltaY * 0.00115;
      lastInteractionAt = performance.now();
    }
    ...
    interactionSurface.addEventListener("dblclick", handleDoubleClick);
    ...
    let lastKeyPulse = finiteNumber(feedingRef.current.pulsoTecla, 0);
    let lastAbsorptionPulse = finiteNumber(
      feedingRef.current.pulsoAbsorcion,
      0,
    );
    let lastDeletionPulse = finiteNumber(feedingRef.current.pulsoBorrado, 0);
    let keyShock = 0;
    ...
    function animate(timestamp = performance.now()) {
      const safeTimestamp = finiteNumber(timestamp, performance.now());
      const frameDelta = clampNumber(
        (safeTimestamp - previousFrameAt) / 1000,
        0,
        0.05,
        0.016,
      );
      previousFrameAt = safeTimestamp;
      const elapsed = Math.max(0, (safeTimestamp - animationStartedAt) / 1000);
      const pulse =
      ...
      const verbalBiomass = clampNumber(feeding.biomasaVerbal, 0, 1, 0);
      const keyPulseValue = finiteNumber(feeding.pulsoTecla, 0);
      const absorptionPulseValue = finiteNumber(feeding.pulsoAbsorcion, 0);
      const deletionPulseValue = finiteNumber(feeding.pulsoBorrado, 0);

      if (keyPulseValue !== lastKeyPulse) {
        keyShock = 1;
        targetManualYaw += 0.018;
        lastKeyPulse = keyPulseValue;
      }
      if (absorptionPulseValue !== lastAbsorptionPulse) {
        absorptionShock = 1;
        targetManualYaw += 0.11;
        targetManualPitch = THREE.MathUtils.clamp(
          targetManualPitch - 0.025,
          -0.58,
          0.58,
        );
        lastAbsorptionPulse = absorptionPulseValue;
      }
      if (deletionPulseValue !== lastDeletionPulse) {
        deletionShock = 1;
        targetManualYaw -= 0.045;
        lastDeletionPulse = deletionPulseValue;
      }

      const recentlyTouched = safeTimestamp - lastInteractionAt < 1450;

      if (!dragging) {
        targetManualYaw += yawVelocity;
        targetManualPitch = THREE.MathUtils.clamp(
          targetManualPitch + pitchVelocity,
          -0.58,
          0.58,
        );
        ...
        if (!recentlyTouched && autoRotationEnabled) {
          // Slow but unmistakable: one full turn in roughly 24 seconds.
          autoYaw += frameDelta * (0.26 + verbalBiomass * 0.1);
        }
      }

      const rotationEase = Math.min(1, frameDelta * 8.5);
      manualYaw += (targetManualYaw - manualYaw) * rotationEase;
      manualPitch += (targetManualPitch - manualPitch) * rotationEase;

      // Rotate only the anatomical embryo. The capsule and laboratory
      // remain fixed, making the rotation visually unmistakable.
      embryoPivot.rotation.y =
        autoYaw +
        manualYaw +
        absorptionShock * 0.055 -
        deletionShock * 0.025;
      embryoPivot.rotation.x =
        manualPitch * 0.86 +
        Math.sin(elapsed * 0.18) * 0.012;
      embryoPivot.rotation.z =
        -manualPitch * 0.11 +
        Math.sin(elapsed * 0.11) * 0.008;

      rotationRig.rotation.set(0, 0, 0);

      keyShock *= 0.76;
      absorptionShock *= 0.88;
      deletionShock *= 0.8;
      ...
      const targetBodyScale =
        ...
        verbalBiomass * 0.48 +
        keyShock * 0.055 +
        absorptionShock * 0.16 -
        deletionShock * 0.075;
      const currentBodyScale = finiteNumber(root.scale.x, 1);
      const smoothBodyScale = clampNumber(
        currentBodyScale + (targetBodyScale - currentBodyScale) * 0.085,
        0.72,
        1.72,
        1,
      );
      ...
      const activeColor =
        habitatColors[feeding.habitatEntrante] ?? habitatColors.memoria;
      keyLight.color.lerp(
      ...
      root.rotation.y +=
        (targetRotation.y + Math.sin(elapsed * 0.09) * 0.055 +
          keyShock * 0.035 - deletionShock * 0.028 - root.rotation.y) *
        0.04;
      root.rotation.z +=
        ((keyShock - deletionShock) * 0.025 - root.rotation.z) * 0.08;

      const targetCameraZ = clampNumber(
        4.65 - verbalBiomass * 0.58 - absorptionShock * 0.16,
        3.65,
        5.2,
        4.65,
      );
      const currentCameraZ = finiteNumber(camera.position.z, 4.65);
      camera.position.z =
      ...
      if (embryo) {
        const embryoBreath =
          ...
          Math.sin(elapsed * 1.2) *
            (0.006 + energy * 0.00003) *
            stageFactor +
          keyShock * 0.04 +
          absorptionShock * 0.13 -
          deletionShock * 0.055;

        const baseEmbryoScale = finiteNumber(
          embryo.userData.baseScale,
          finiteNumber(embryo.scale.x, 1),
        );
        const safeEmbryoBreath = clampNumber(
          embryoBreath,
          0.78,
          1.36,
          1,
        );
        embryo.scale.setScalar(baseEmbryoScale * safeEmbryoBreath);
        embryo.userData.lastBreathScale = safeEmbryoBreath;
        embryo.position.y =
          -0.08 +
          Math.sin(elapsed * 0.62) * 0.025 * stageFactor +
          absorptionShock * 0.11 -
          deletionShock * 0.07;
        embryo.rotation.y =
          -0.56 + Math.sin(elapsed * 0.21) * 0.028 +
          keyShock * 0.045 - deletionShock * 0.035;
        embryo.rotation.z =
          Math.sin(elapsed * 0.27) * 0.01 +
          absorptionShock * 0.035 - deletionShock * 0.025;
      }

      if (
        !Number.isFinite(root.scale.x) ||
        !Number.isFinite(root.scale.y) ||
        !Number.isFinite(root.scale.z)
      ) {
        root.scale.setScalar(1);
      }

      if (
        !Number.isFinite(embryoPivot.rotation.x) ||
        !Number.isFinite(embryoPivot.rotation.y) ||
        !Number.isFinite(embryoPivot.rotation.z)
      ) {
        embryoPivot.rotation.set(0, 0, 0);
        autoYaw = 0;
        manualYaw = 0;
        manualPitch = 0;
        targetManualYaw = 0;
      ...
      if (
        embryo &&
        (!Number.isFinite(embryo.scale.x) ||
          !Number.isFinite(embryo.scale.y) ||
          !Number.isFinite(embryo.scale.z))
      ) {
        embryo.scale.setScalar(
          finiteNumber(embryo.userData.baseScale, 1),
        );
      }
  ...
  return (
    <div className={styles.embryoScene} ref={mountRef}>
      ...
      <div className={styles.embryoRotateHint} aria-hidden="true">
        <span>↔</span>
        ARRASTRA SOBRE EL EMBRIÓN · GEOMETRÍA ESTABLE
      </div>

      <div
        className={styles.embryoRotationControls}
        aria-label="Controles de rotación del embrión"
      >
        <button
          type="button"
          ...
          onClick={() =>
            mountRef.current?.dispatchEvent(
              new CustomEvent("organismo-rotation-command", {
                detail: -1,
              }),
            )
          }
