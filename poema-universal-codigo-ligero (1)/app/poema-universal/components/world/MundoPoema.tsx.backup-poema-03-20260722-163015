...
const BOOK_POEMS: PoemEntry[] = [
  {
    id: "jose-antonio-balo-naveiro-01",
...
function Poet({
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime() * 1.15 + phase;

    if (group.current) {
      group.current.position.y =
        position[1] + Math.sin(t) * 0.035;

      group.current.rotation.y =
        rotation + Math.sin(t * 0.35) * 0.06;
    }

    if (arm.current) {
      arm.current.rotation.x =
        -0.35 + Math.sin(t) * 0.28;
    }
  });
  ...
  return (
    ...
    >
      <mesh position={[0, 0.64, 0]} castShadow>
        <cylinderGeometry args={[0.2, 0.31, 0.92, 12]} />
        <meshStandardMaterial color={color} roughness={0.92} />
      </mesh>

      <mesh position={[0, 1.25, 0]} castShadow>
        <sphereGeometry args={[0.2, 18, 18]} />
        <meshStandardMaterial color="#b78e70" roughness={0.9} />
      </mesh>

      <group ref={arm} position={[0.2, 0.88, -0.04]}>
        <mesh rotation={[0, 0, -0.45]} castShadow>
          <cylinderGeometry args={[0.05, 0.065, 0.55, 10]} />
          <meshStandardMaterial color="#a98268" roughness={0.9} />
        </mesh>
      </group>
