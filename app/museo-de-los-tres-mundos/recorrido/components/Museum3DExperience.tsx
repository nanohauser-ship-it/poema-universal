...
function createArtwork(
  ...
  if (artwork.imageUrl) {
    artworkTextureLoader.load(
      ...
      (error) => {
        console.error(
          `No se pudo cargar la obra ${artwork.id}:`,
          error
        );
      }
    );
  ...
  const isCarroDePaja =
    artwork.id === "el-carro-de-paja";

  const light = new THREE.PointLight(
    hexToNumber(artwork.accent),
    isCarroDePaja ? 0.62 : 1.1,
    isCarroDePaja ? 4.4 : 5.5,
    2
  );
...
function createMuseumRoom(
  ...
  if (roomId === "bielka") {
    ...
    /*
     * El arco queda detrás de la obra central.
     * La obra está aproximadamente en z = -5.02.
     * El arco se sitúa en z = -5.16 para no atravesar
     * ni el marco ni el plano de la imagen.
     */
    const thresholdArch =
      new THREE.Mesh(
    ...
    thresholdArch.castShadow = false;
    thresholdArch.receiveShadow = true;
...
export default function Museum3DExperience() {
  ...
  const selectedArtworkRef =
    useRef<MuseumArtwork | null>(null);

  const selectedArtworkObjectRef =
    useRef<ArtworkObject | null>(null);

  const artworkObjectsRef =
    useRef<ArtworkObject[]>([]);
  ...
  function closeArtwork() {
    ...
    selectedArtworkObjectRef.current = null;

    setSelectedArtwork(null);
  }

  function navigateArtwork(
    direction: -1 | 1
  ) {
    const current =
      selectedArtworkRef.current;
    ...
    const roomArtworks =
      museumArtworks
        .filter(
          (artwork) =>
            artwork.room === current.room
        )
        .slice(0, 3);

    const currentIndex =
      roomArtworks.findIndex(
        (artwork) =>
          artwork.id === current.id
      );
    ...
    const nextArtwork =
      roomArtworks[nextIndex];

    const nextArtworkObject =
      artworkObjectsRef.current.find(
        (entry) =>
          entry.artwork.id ===
            nextArtwork.id &&
          entry.artwork.room ===
    ...
    if (nextArtworkObject) {
      selectedArtworkObjectRef.current =
        nextArtworkObject;
    }

    openArtwork(nextArtwork);
  }
  ...
  useEffect(() => {
    ...
    const artworkWorldPosition =
      new THREE.Vector3();

    const artworkNormal =
      new THREE.Vector3();

    const artworkQuaternion =
    ...
    function animate() {
      ...
      if (focusedArtwork) {
        ...
        focusedArtwork.frame.getWorldPosition(
          artworkWorldPosition
        );

        focusedArtwork.group.getWorldQuaternion(
          artworkQuaternion
        );
        ...
        if (
          focusedArtwork.artwork.id ===
          "el-vuelo-de-icaro-bielka"
        ) {
          desiredLookTarget.y -= 0.12;
        }
        ...
        const focusDistance =
          ...
          "el-vuelo-de-icaro-bielka"
            ? 4.65
            : focusedArtwork.artwork.aspect ===
                "landscape"
              ? 3.8
              : 3.45;

        desiredCameraPosition
          .copy(artworkWorldPosition)
          .addScaledVector(
            artworkNormal,
            focusDistance
          );

        desiredCameraPosition.y += 0.08;

        const cameraEase =
          reducedMotion ? 0.2 : 0.055;
          ...
          <div className="mt-8 border-t border-white/10 pt-5">
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="border border-white/10 px-4 py-3 text-[7px] uppercase tracking-[0.24em] text-white/45 transition hover:border-white/30 hover:text-white"
              >
                ← Anterior
              </button>
            ...
            <button
              type="button"
              onClick={closeArtwork}
              className="mt-2 w-full border border-white/10 px-4 py-3 text-[7px] uppercase tracking-[0.27em] text-white/32 transition hover:border-white/30 hover:text-white"
            >
              Volver a la sala
            </button>

            <p className="mt-4 text-center text-[6px] uppercase tracking-[0.24em] text-white/20">
              También puedes usar ← → y Esc
            </p>
          </div>
        </article>
      )}
