...
export default function Museum3DExperience() {
  ...
  const [exhibitionAct, setExhibitionAct] =
    useState<0 | 1>(0);

  const [
    isExhibitionTransitioning,
    setIsExhibitionTransitioning,
  ] = useState(false);
  ...
  function selectRoom(roomId: MuseumRoomId) {
    ...
    setExhibitionAct(0);
    setIsExhibitionTransitioning(false);
  }
  ...
  }, [selectedArtwork, exhibitionAct]);

  function changeExhibitionAct(
    nextAct: 0 | 1
  ) {
    if (
      nextAct === exhibitionAct ||
      isExhibitionTransitioning
    ) {
      return;
    }
    ...
    setSelectedArtwork(null);
    setIsExhibitionTransitioning(true);

    window.setTimeout(() => {
      setExhibitionAct(nextAct);
  ...
  function showPreviousAct() {
    changeExhibitionAct(0);
  }

  function showNextAct() {
    changeExhibitionAct(1);
  }
  ...
  return (
    <section className="relative min-h-[900px] overflow-hidden bg-[#050608] text-[#f0e8dc]">
      ...
      <div
        aria-hidden="true"
        className={`pointer-events-none absolute inset-0 z-[45] bg-[#050608] transition-opacity duration-[420ms] ${
          isExhibitionTransitioning
            ? "opacity-100"
            : "opacity-0"
        }`}
      ...
      <div className="pointer-events-none relative z-10 mx-auto flex min-h-[900px] max-w-[1550px] flex-col justify-between px-5 py-8 sm:px-8 lg:px-12">
        ...
        {!selectedArtwork && (
          <div className="pointer-events-auto absolute bottom-[106px] left-1/2 z-30 flex w-[min(720px,calc(100%-2.5rem))] -translate-x-1/2 flex-wrap items-center justify-between gap-3 border border-white/10 bg-[#050608]/82 px-4 py-3 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[7px] uppercase tracking-[0.3em] text-white/38">
              Acto {exhibitionAct + 1} de 2
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={showPreviousAct}
              disabled={
                exhibitionAct === 0 ||
                isExhibitionTransitioning
              }
              className="border border-white/10 px-4 py-2 text-[7px] uppercase tracking-[0.24em] text-white/38 transition enabled:hover:border-white/30 enabled:hover:text-white disabled:cursor-default disabled:opacity-20"
            >
              ← Anterior
            </button>
      ...
      {selectedArtwork && (
        <article className="pointer-events-auto absolute right-5 top-[132px] z-50 w-[min(390px,calc(100%-2.5rem))] border border-white/12 bg-[#050608]/92 p-6 shadow-[0_30px_100px_rgba(0,0,0,0.65)] backdrop-blur-2xl sm:right-8 lg:right-12">
