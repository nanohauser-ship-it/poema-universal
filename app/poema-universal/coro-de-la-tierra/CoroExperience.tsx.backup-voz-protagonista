...
function VoiceSphere({
  ...
  useEffect(() => {
    ...
    const selectedPoint = points[hashText(selectedId) % points.length];
    const desiredY = Math.atan2(selectedPoint.x, selectedPoint.z);
    const zAfterY = selectedPoint.x * Math.sin(desiredY) + selectedPoint.z * Math.cos(desiredY);
    const desiredX = Math.atan2(selectedPoint.y, zAfterY);
    const currentY = rotationRef.current.y;
    const shortestY = currentY + Math.atan2(
      Math.sin(desiredY - currentY),
      Math.cos(desiredY - currentY),
    );
    targetRotationRef.current = { x: desiredX, y: shortestY };
  }, [points, selectedId]);
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      const breath = 1 + energy * 0.035;
      const lightPulse = 0.45 + energy * 0.55;

      const sphereRadius =
        Math.min(width, height) * 0.385 * breath;

      if (!dragRef.current.active) {
        const target = targetRotationRef.current;
        if (target) {
          const differenceX = target.x - rotationRef.current.x;
          const differenceY = target.y - rotationRef.current.y;
          rotationRef.current.x += differenceX * 0.065;
          rotationRef.current.y += differenceY * 0.065;
          velocityRef.current = { x: 0, y: 0 };
          if (Math.abs(differenceX) < 0.002 && Math.abs(differenceY) < 0.002) {
      ...
      halo.addColorStop(
        0,
        `rgba(255, 230, 174, ${0.10 + energy * 0.28})`,
      );
      halo.addColorStop(
        0.58,
        `rgba(213, 168, 88, ${0.04 + energy * 0.16})`,
      );
      ...
      for (let orbit = 0; orbit < 4; orbit += 1) {
        ...
        context.lineWidth = 0.65 + energy * 1.45;
        context.stroke();
      ...
      voices.forEach((voice, index) => {
        ...
        context.shadowBlur = glow + energy * 18;
...
function UploadStudio({
  ...
  const updateDraft = (key: string, field: "name" | "place" | "language" | "excerpt" | "translation", value: string) => {
              ...
              <article className={`draft-card status-${draft.status}`} key={draft.key}>
                ...
                <div className="draft-fields">
                  ...
                  <div className="field-grid">
                    ...
                    <label className="excerpt-field">Traducción<textarea value={draft.translation} onChange={(event) => updateDraft(draft.key, "translation", event.target.value)} placeholder="Traducción opcional del fragmento…" rows={2} /></label>
                  </div>
...
export default function CoroExperience() {
  ...
  const selectedIndex = selected ? voices.findIndex((voice) => voice.id === selected.id) : -1;
  ...
  const navigateVoice = useCallback((direction: -1 | 1) => {
    ...
    const currentIndex = selectedId
      ? voices.findIndex((voice) => voice.id === selectedId)
      : 0;
    const nextIndex = (Math.max(0, currentIndex) + direction + voices.length) % voices.length;
    selectVoice(voices[nextIndex].id);
  }, [selectVoice, selectedId, voices]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft") {
        ...
        event.preventDefault();
        navigateVoice(1);
      } else if (event.key === "Escape" && archiveOpen) {
        setArchiveOpen(false);
      }
    };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [archiveOpen, navigateVoice]);
  ...
  return (
    <main className="coro-room">
      ...
      <div className="sphere-stage">
        <VoiceSphere voices={voices} selectedId={selected?.id ?? null} onSelect={selectVoice} energyRef={energyRef} />
        ...
        {loading ? (
          ...
          <>
            <div className="voice-card-head">
              <nav className="voice-navigation" aria-label="Navegación entre voces">
                <button type="button" onClick={() => navigateVoice(-1)} disabled={voices.length < 2} aria-label="Voz anterior">←</button>
                <span className="voice-index">VOZ {String(selectedIndex + 1).padStart(2, "0")} DE {String(voices.length).padStart(2, "0")}</span>
                <button type="button" onClick={() => navigateVoice(1)} disabled={voices.length < 2} aria-label="Voz siguiente">→</button>
              </nav>
            ...
            {selected.translation && <p className="voice-translation"><span>TRADUCCIÓN</span>{selected.translation}</p>}
            <button type="button" className="all-voices-button" onClick={() => setArchiveOpen(true)}>VER TODAS LAS VOCES</button>
      ...
      {archiveOpen && (
        <>
          <button type="button" className="archive-scrim" onClick={() => setArchiveOpen(false)} aria-label="Cerrar archivo de voces" />
          <aside className="voice-archive" aria-label="Archivo completo de voces">
            <header>
              <div>
                <p>ARCHIVO CORAL</p>
                <h2>Todas las voces</h2>
              </div>
              <button type="button" onClick={() => setArchiveOpen(false)} aria-label="Cerrar archivo">×</button>
            </header>
            <div className="voice-archive-list">
              {voices.map((voice, index) => (
                <button
                  type="button"
                  key={voice.id}
                  className={voice.id === selected?.id ? "is-selected" : ""}
                  onClick={() => {
                ...
                >
                  ...
                  <small>{[voice.place, voice.language].filter(Boolean).join(" · ") || "Procedencia por registrar"}</small>
                </button>
              ))}
            </div>
          </aside>
