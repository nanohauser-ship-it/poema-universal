...
const SaltDogPresence = dynamic(
  () => import("./SaltDogPresence").then((module) => module.SaltDogPresence),
  { ssr: false, loading: () => <div className={styles.loading}>La presencia está emergiendo…</div> }
);
...
export function CreatureChamber({ creature }: { creature: CanonicalCreature }) {
  ...
  function renderPresence() {
    if (creature.slug === "el-perro-de-sal-que-guarda-la-ausencia") {
      return <SaltDogPresence />;
    }
    return <SpiderPresence />;
  }

  return (
    <section className={styles.chamber}>
      <div className={styles.topbar}>
        <p>Bestiario Poético · Cámara {creature.number.toString().padStart(3, "0")}</p>
        <div>
      ...
      <div className={styles.viewport}>
        ...
        <div className={styles.title}>
          <span>{creature.number.toString().padStart(3, "0")}</span>
          <h2>{creature.name}</h2>
        </div>
      </div>
