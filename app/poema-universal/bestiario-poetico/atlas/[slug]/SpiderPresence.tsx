  ...
  return (
    <group ref={group}>
      ...
      {legs.map((leg, index) => {
        ...
        return (
          <group key={index}>
            <mesh castShadow position={[(x + outerX) / 2, 0.1, (leg.z + outerZ) / 2]} rotation={[0, Math.atan2(outerX - x, outerZ - leg.z), Math.PI / 2 + leg.bend]}>
              <cylinderGeometry args={[0.065, 0.09, 1.08, 12]} />
              <meshStandardMaterial color="#211c18" roughness={0.86} />
            </mesh>
            <mesh castShadow position={[outerX + leg.side * 0.35, -0.18, outerZ + 0.15]} rotation={[0, Math.atan2(leg.side * 0.7, 0.3), Math.PI / 2 - leg.side * 0.22]}>
...
export function SpiderPresence() {
  return (
    <Canvas camera={{ position: [0, 1.25, 5.7], fov: 34 }} dpr={[1, 1.7]} shadows>
      ...
      <OrbitControls enablePan={false} minDistance={4.1} maxDistance={7} minPolarAngle={0.85} maxPolarAngle={1.75} autoRotate autoRotateSpeed={0.28} />
    </Canvas>
  );
}
