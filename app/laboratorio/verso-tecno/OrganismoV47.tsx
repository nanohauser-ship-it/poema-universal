...
import LiteraryNotebook, {
  type NotebookInsertion,
} from "./organismo/LiteraryNotebook";
import WordArchive, {
  type ArchivedWord,
...
export default function OrganismoV47() {
  ...
  const notebookInsertionIdRef = useRef(0);
  ...
  // EMBRIÓN V6.2.1 · DESBLOQUEO DE GRAVEDAD
  // La consola ya no depende únicamente del campo de escritura. La membrana
  // permanente y el vivero también pueden convertirse en centros de gravedad.
  const feedingSymbolicWords = useMemo(
    () => extractWritingWords(feedingText),
    [feedingText],
  );
  const selectableSymbolicWords = useMemo(() => {
    const ordered = [
      ...wordArchive.slice(-24).map((entry) => entry.value),
      ...savedImpulses.slice(-12).map((impulse) => impulse.value),
      ...feedingSymbolicWords,
      symbolicNucleus,
    ].filter(Boolean);
  ...
  }, [wordArchive, savedImpulses, feedingSymbolicWords, symbolicNucleus]);
  const consoleSymbolicText = useMemo(
    () => selectableSymbolicWords.join(" "),
    [selectableSymbolicWords],
  );
  ...
  function sendToNotebook(text: string, source: string) {
    const clean = text.trim();
    if (!clean) return;
    const id = notebookInsertionIdRef.current + 1;
    notebookInsertionIdRef.current = id;
    setNotebookInsertion({ id, text: clean, source });
  }

  function insertImpulse(impulse: SymbolicImpulse) {
    sendToNotebook(
      impulse.value,
      `${impulse.kind} · ${impulse.evidence} · ${impulse.sourceLabels.slice(0, 2).join(" / ") || "sin fuente"}`,
    );
    setLastWord(impulse.value);
    setFeedingHabitat(impulse.habitat as Habitat);
    setAbsorptionPulse((value) => value + 2);
    notify("El impulso fue enviado a la Libreta de Escritura.");
  }

  function insertAssembly(entries: ArchivedWord[]) {
    if (!entries.length) return;
    const passage = entries.map((entry) => entry.value).join(" · ");
    sendToNotebook(passage, "Mesa de montaje");
    ...
    notify(`${entries.length} formas fueron enviadas a la Libreta de Escritura.`);
  }
  ...
  function recallArchivedWord(entry: ArchivedWord) {
    // V6.2.1: tocar una forma significa convertir ESA forma en gravedad.
    // Antes intentábamos reconstruir su núcleo histórico y el efecto de
    // selección podía deshacer el clic inmediatamente.
    ...
    setAbsorptionPulse((value) => value + 2);
    notify(`«${entry.value}» es ahora el centro de gravedad.`);
  }
  ...
  function incubate() {
    const matter = poem.trim();

    // V6.2.2 · ACCESO INTERIOR
    // Una palabra ya es materia poética suficiente para engendrar presencia.
    if (!matter) {
      notify("Escribe al menos una palabra para incubar el organismo.");
      return;
    }

    const next = incubar(name, matter);
    persist(next);
    setProgress(0);
    setPlayingVisual(false);
  ...
  // EMBRIÓN V6.3 · LIBRETA LITERARIA
  return (
