...
export function validateScenographyBrief(input: unknown): ScenographyBrief | null {
  ...
  const takeMode = input.takeMode === "LIVE" || input.takeMode === "CURATED"
    ? input.takeMode
    : null;
  const preferredArchetype = ARCHITECTURE_ARCHETYPES.find(
    (item) => item === input.preferredArchetype
  );
  ...
  return {
    ...
    seed,
    takeMode,
    ...(safeString(input.theme, 96)
      ? { theme: safeString(input.theme, 96)! }
      : {}),
    ...(safeString(input.narrativeIntent, 280)
      ? { narrativeIntent: safeString(input.narrativeIntent, 280)! }
      : {}),
    ...(safeId(input.zoneId) ? { zoneId: safeId(input.zoneId)! } : {}),
