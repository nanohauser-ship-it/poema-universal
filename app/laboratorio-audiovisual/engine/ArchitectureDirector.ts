...
export class ArchitectureDirector {
  ...
  private requestedCameraAnchor: CameraAnchorBlueprint["id"] = "MASTER";
  ...
  createThematicTake(
    ...
    this.requestedCameraAnchor = zone.cameraAnchor;
    const brief = this.createBrief(
      journeyId,
      createExperienceInstanceId(),
      zone.seed,
      ...
      {
        theme,
        narrativeIntent: intention,
        zoneId: zone.id,
        preferredArchetype: zone.archetype,
        emotion: intention,
        rhythm: zone.rhythm,
        spatialIntent: zone.spatialIntent,
        performerRequired,
      }
    );
  ...
  private createBrief(
    ...
    return {
      ...
      seed,
      takeMode,
      ...(direction.theme ? { theme: direction.theme } : {}),
      ...(direction.narrativeIntent
        ? { narrativeIntent: direction.narrativeIntent }
        : {}),
      ...(direction.zoneId ? { zoneId: direction.zoneId } : {}),
      ...(direction.preferredArchetype
        ? { preferredArchetype: direction.preferredArchetype }
        : {}),
      emotion: direction.emotion ?? "presencia, profundidad y espera",
      rhythm: direction.rhythm ?? "cinematográfico, lento y respirado",
      spatialIntent:
        direction.spatialIntent ??
        "arquitectura museística filmable alrededor del performer",
      performerRequired: direction.performerRequired ?? true,
  ...
  private async generateTake(
    ...
    try {
      const planned = allowRemotePlanning
        ...
        : {
            blueprint: generateLocalBlueprint(
              brief,
      ...
      try {
        const fallback = generateLocalBlueprint(
          brief,
          brief.preferredArchetype
        );
