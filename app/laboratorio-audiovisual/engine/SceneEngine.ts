...
import { ArchitectureDirector } from "./ArchitectureDirector";
...
import type { ThematicJourneyZone } from "../scenography/thematicJourney";
import type {
...
export class SceneEngine implements ExperienceRuntime {
  ...
  private readonly architectureDirector: ArchitectureDirector;
  ...
  constructor(private readonly mount: HTMLDivElement) {
    ...
    this.architectureManager = new ArchitectureManager(this.mediaManager);
    ...
    this.architectureDirector = new ArchitectureDirector(
      this.architectureManager,
      this.cameraDirector
    );
  ...
  subscribeArchitectureState(
    listener: (state: ArchitectureDirectorState) => void
  ): () => void {
    return this.architectureDirector.subscribe(listener);
  }

  createNewTake(): Promise<ArchitectureBlueprint | null> {
    return this.architectureDirector.createNewTake();
  }

  reuseTake(seed: string): Promise<ArchitectureBlueprint | null> {
    return this.architectureDirector.reuseTake(seed);
  }

  regenerateArchitecture(): Promise<ArchitectureBlueprint | null> {
    return this.architectureDirector.regenerateArchitecture();
  }
  ...
  createThematicTake(
    ...
    zone: ThematicJourneyZone
  ): Promise<ArchitectureBlueprint | null> {
    return this.architectureDirector.createThematicTake(
      journeyId,
      theme,
      intention,
      performerRequired,
  ...
  setArchitectureMediaSlot(
    ...
    this.architectureManager.setMediaSlot(slotId, content);
  }

  setArchitectureCameraAnchor(
    id: CameraAnchorBlueprint["id"],
    outputProfile?: OutputProfileId
  ): boolean {
    return this.architectureDirector.setCameraAnchor(id, outputProfile);
  }
  ...
  setExperienceSurfaceMedia(
    experience: SceneMode,
    id: SurfaceKey,
    content: MediaContent | null
  ): void {
    if (this.disposed) {
    ...
    this.experienceDirector.setSurfaceMediaOverride(
      experience,
      id,
      content
    );
  ...
  dispose(): void {
    ...
    this.architectureDirector.dispose();
    this.architectureManager.dispose();
  ...
  private readonly animate = (): void => {
    ...
    this.architectureDirector.update(deltaTime);
