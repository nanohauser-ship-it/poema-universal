...
export class CameraDirector {
  ...
  private readonly renderedPositionTarget = new THREE.Vector3();
  ...
  update(deltaTime: number): void {
    ...
    this.dolly = THREE.MathUtils.lerp(this.dolly, this.targetDolly, lookAlpha);
    this.viewDirection
      .copy(this.targetPosition)
      .sub(this.targetLookAt)
      .normalize();
    this.renderedPositionTarget
      .copy(this.targetPosition)
      .addScaledVector(this.viewDirection, this.dolly);
    this.camera.position.lerp(this.renderedPositionTarget, cameraAlpha);
    ...
    const lookDistance = Math.max(this.viewDirection.length(), 0.5);
    this.viewDirection
      .normalize()
      .applyAxisAngle(this.upAxis, this.yaw);
    this.renderedLookAt
      .copy(this.camera.position)
      .addScaledVector(this.viewDirection, lookDistance);
    this.renderedLookAt.y -= this.pitch * lookDistance * 0.72;
  ...
  private readonly onPointerMove = (event: PointerEvent): void => {
    ...
    if (Math.abs(this.targetYaw) > Math.PI * 4) {
  ...
  private readonly onWheel = (event: WheelEvent): void => {
    ...
    this.targetDolly = THREE.MathUtils.clamp(
      this.targetDolly + event.deltaY * 0.006,
      -3.5,
      6.5
    );
  };
