...
`thematicJourney.ts` transforma un tema y una intención en un recorrido reproducible
de tres zonas: Umbral, Inmersión y Presencia. Cada zona conserva su propia semilla,
arquetipo, ritmo, intención espacial y anchor de cámara. Al activarla, esos datos se
incorporan al `ScenographyBrief`; la planificación remota interpreta el tema y el
fallback local mantiene el arquetipo elegido.

Los slots `WALL_PRIMARY`, `WALL_LEFT`, `WALL_RIGHT`, `FLOOR_PRIMARY` y
`CEILING_PRIMARY` se conectan a `MediaManager`. De este modo, vídeos e imágenes
cargados localmente pueden continuar en la sala base, el corredor del portal y las
superficies de una arquitectura generada sin crear otro renderer ni otro loop.
