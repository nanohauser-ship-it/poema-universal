...
export function standardSurfaces(
  ...
  const floorSlot = slotIds[3];
  const ceilingSlot = slotIds[4];

  const surfaces: ProjectionSurfaceBlueprint[] = [
    {
      id: "SURFACE_BACK",
      slotId: backSlot,
  ...
  if (floorSlot) {
    surfaces.push({
      id: "SURFACE_FLOOR",
      slotId: floorSlot,
      position: [0, 0.035, -roomDepth * 0.08],
      rotation: [-Math.PI / 2, 0, 0],
      size: [roomWidth * 0.72, roomDepth * 0.68],
      opacity: 0.68,
    });
  }
  ...
  if (ceilingSlot) {
    surfaces.push({
      id: "SURFACE_CEILING",
      slotId: ceilingSlot,
      position: [0, roomHeight - 0.045, -roomDepth * 0.08],
      rotation: [Math.PI / 2, 0, 0],
      size: [roomWidth * 0.68, roomDepth * 0.64],
      opacity: 0.58,
    });
  }
