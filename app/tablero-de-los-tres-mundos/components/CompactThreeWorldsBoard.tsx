...
export default function ThreeWorldsBoard() {
  ...
  const [hoveredPieceId, setHoveredPieceId] =
    useState<PieceId | null>(null);

  const [selectedPieceId, setSelectedPieceId] =
    useState<PieceId | null>(null);

  const selectedPieceIdRef =
    useRef<PieceId | null>(null);
  ...
  useEffect(() => {
    ...
    /*
     * Doce piezas fundacionales.
     */

    const pieceScene = createWorldPieces();

    root.add(pieceScene.group);

    const interactivePieces =
      pieceScene.interactiveObjects;
    ...
    let hoveredPieceObject: THREE.Object3D | null =
      null;

    let hoveredPieceLocal: PieceId | null = null;
    ...
    function findPieceParent(
      object: THREE.Object3D | null
    ) {
      let current = object;

      while (current) {
        if (current.userData.interactivePiece) {
          return current;
        }
    ...
    function updateHoveredPiece(
      ...
      if (object === hoveredPieceObject) {
        return;
      }

      if (hoveredPieceObject) {
        const baseScale =
          hoveredPieceObject.userData.baseScale || 1;

        hoveredPieceObject.scale.setScalar(baseScale);
      }

      hoveredPieceObject = object;

      const nextId = object
        ? (object.userData.pieceId as PieceId)
        : null;

      hoveredPieceLocal = nextId;
      setHoveredPieceId(nextId);

      if (object) {
        const baseScale =
          object.userData.baseScale || 1;

        object.scale.setScalar(baseScale * 1.09);
    ...
    function updatePointerPosition(
      ...
      const interactive = findPieceParent(
        intersections[0]?.object || null
      );

      updateHoveredPiece(interactive);
    }
    ...
    function finishDragging(
      ...
      renderer.domElement.style.cursor =
        hoveredPieceLocal ? "pointer" : "grab";
    }

    function handlePointerUp(
      event: PointerEvent
    ) {
      const wasClick = dragDistance < 7;

      finishDragging(event);

      if (wasClick && hoveredPieceLocal) {
        const piece = pieceById[hoveredPieceLocal];

        setSelectedPieceId(hoveredPieceLocal);

        selectedWorldRef.current = piece.world;
        setSelectedWorld(piece.world);
      }
    }
    ...
    function animate() {
      ...
      const desiredCameraY =
        activeWorld.height + 5.15;

      camera.position.y +=
        (desiredCameraY - camera.position.y) *
        0.035;

      const desiredX = 9.4 + pointerX * 0.5;
      const desiredZ = 11.8 + pointerY * 0.24;
  ...
  const activePiece = activePieceId
    ? pieceById[activePieceId]
    : null;

  return (
    <section className="relative h-[620px] overflow-hidden bg-[#07090c] text-[#f0e8dc] sm:h-[680px]">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0"
        style={{
          background:
            "radial-gradient(circle at 50% 44%, rgba(226,213,187,0.09), transparent 30%), radial-gradient(circle at 18% 15%, rgba(130,176,188,0.08), transparent 28%), radial-gradient(circle at 80% 82%, rgba(150,74,40,0.12), transparent 30%)",
        }}
      />
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start justify-between p-5 sm:p-7">
        <div>
          <p className="text-[7px] uppercase tracking-[0.48em] text-[#c7a467]/70">
            Tablero de los tres mundos
          </p>
        </div>

        <p className="hidden text-[6px] uppercase tracking-[0.28em] text-white/22 sm:block">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 p-4 sm:p-6">
        {activePiece && (
          <div className="mb-4 flex justify-center">
            <div className="border border-white/10 bg-black/40 px-5 py-3 backdrop-blur-md">
              <p
                className="text-center text-[6px] uppercase tracking-[0.3em]"
                style={{
                  color: worldById[activePiece.world].accent,
                }}
              >
                {activePiece.archetype}
              </p>

              <p className="mt-2 text-center font-serif text-base italic text-white/65">
                {activePiece.name}
              </p>
            </div>
          </div>
        ...
        <div className="pointer-events-auto mx-auto grid max-w-[850px] border-l border-t border-white/10 sm:grid-cols-3">
          {worlds.map((world) => {
            ...
            return (
              <button
                ...
                className="group border-b border-r border-white/10 bg-black/20 px-4 py-4 text-left backdrop-blur-sm transition hover:bg-white/[0.04]"
              >
                <span
                  className="text-[6px] uppercase tracking-[0.34em]"
                  style={{
                    color: active
                      ? world.accent
                ...
                >
                  {world.number} · {world.material}
                </span>

                <span className="mt-2 block font-serif text-base text-white/58 transition group-hover:text-white">
                  {world.title}
                </span>
              </button>
            );
