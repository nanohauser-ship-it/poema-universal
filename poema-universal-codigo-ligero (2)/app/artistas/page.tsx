...
import portadas001050 from "@/public/artistas/portadas/manifest-001-050.json";
import portadas051100 from "@/public/artistas/portadas/manifest-051-100.json";
...
const PORTADAS_CANON = new Map(
  [...portadas001050, ...portadas051100].map((portada) => [
    normalizarNombre(portada.name),
    portada,
  ])
);
...
async function getArtistas(): Promise<AtlasArtista[]> {
  ...
  return ((artistasData || []) as ArtistaRaw[])
    .filter(estaVisible)
    .map((artista) => {
      const portadaCanon = PORTADAS_CANON.get(
        normalizarNombre(artista.nombre)
      );
      ...
      return {
        ...
        portada_url: portadaCanon?.publicPath || null,
