...
export async function POST(request: Request) {
  ...
  try {
    ...
    const direction = parseDirection(text, fragmento, mode, body);
    return NextResponse.json({ direction, source: "openai" });
  } catch (error) {
    return NextResponse.json({
      direction: fallbackDirection(fragmento, mode, body),
...
function parseDirection(text: string, fragmento: string, mode: GraphicMode, body: any): Direction {
  const cleaned = text.replace(/^```json\s*/i, "").replace(/```$/i, "").trim();
  try {
    return { ...fallbackDirection(fragmento, mode, body), ...JSON.parse(cleaned) };
  } catch {
    return fallbackDirection(fragmento, mode, body);
  }
}

function fallbackDirection(fragmento: string, mode: GraphicMode, body?: { resumenUnidad?: string; objetivoVisual?: string; notasUnidad?: string; plateHint?: string }): Direction {
  ...
  return {
    accion: body?.resumenUnidad || firstSentence,
    lugar: "Determinar únicamente a partir del fragmento y su contexto.",
    momento: "No fijar una hora si la novela no la establece.",
    personajes: [],
    emocion: mode === "literal" ? "Tensión narrativa directa." : mode === "eliptico" ? "Resonancia, resto o amenaza." : "Tensión contenida y clima interior.",
    elementoDominante: body?.objetivoVisual || "El espacio y la presencia narrativa principal.",
    simbolo: "Ninguno añadido; usar solo símbolos ya presentes en la obra.",
    noRevelar: "No anticipar información posterior.",
    plano: mode === "eliptico" ? "plano detalle o plano vacío" : mode === "literal" ? "plano general o medio con acción legible" : "plano general contenido",
    camara: "altura humana, observación no enfática",
    composicion: `${modeNotes[mode]} ${body?.plateHint || ""}`.trim(),
    ...
    promptImagen: `${modeNotes[mode]} ${body?.objetivoVisual || ""} ${body?.notasUnidad || ""} Novela gráfica literaria, realismo de memoria, composición editorial, luz natural fría, textura analógica fina, sin fantasía decorativa. Escena basada exclusivamente en: ${fragmento.slice(0, 900)}`.trim()
  };
}
